...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yuyueshijianduan');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','yuyueshijianduan');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('yuyueshijianduan', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussyuyueshijianduan/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('yuyueshijianduan', this.detail);
				this.$utils.msg('审核成功');
